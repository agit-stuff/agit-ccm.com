...
Creates a neural commit that:
1. Reads all entries from the staging area
2. Synthesizes a summary from intents and reasoning
3. Creates a content-addressable commit object
4. Links to the current Git HEAD commit
...
1. **Commit after logical units**: Create neural commits at the same time as git commits
